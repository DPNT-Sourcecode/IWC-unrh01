...
def test_promoted_bank_statement_rule_of_three() -> None:
    run_queue(
        [
            call_enqueue("bank_statements", 1, iso_ts(delta_minutes=0)).expect(1),
...
def test_promoted_bank_statement_rule_of_three_longer_delay() -> None:
    run_queue(
        [
            call_enqueue("bank_statements", 1, iso_ts(delta_minutes=0)).expect(1),
            call_enqueue("companies_house", 1, iso_ts(delta_minutes=1)).expect(2),
...
def test_promoted_bank_statement_same_timestamp() -> None:
    run_queue(
        [
            call_enqueue("companies_house", 1, iso_ts(delta_minutes=0)).expect(1),
            call_enqueue("bank_statements", 1, iso_ts(delta_minutes=0)).expect(2),
